...
| [ThoughtProcess](src/components/Chat/CLAUDE.md) | what an assistant did before it answered, folded away | a single disclosure on `Collapsible`, not an Accordion; Figma's Thinking row is `thinking` plus an `icon` slot the application fills |
