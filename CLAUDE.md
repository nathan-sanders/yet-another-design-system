...
16. **Slider** — drag a handle along a track to pick a number, or a pair of them to pick a range.
    ...
    **32px row, 4px track, 24px control, 16/20 handle, 2×8 tick nubs, 8px gaps and 10px reserved
    ...
    an instance of an **Input** component that lives elsewhere in the file. **Input landed as (18)**,
...
18. **Input / InputGroup** — a single line of free text, and the same field with things attached to
...
20. **Select** — pick one value from a list too long to show all at once. Mirrors Figma nodes
...
21. **Card** — native container using `bg-surface-card-primary`, `border-surface-border`, elevation.
22. **List Item** — variants/states; native, styled.
23. **Table Cell** — native, styled.
24. Then: Indicator, Chart Legend Buttons, Carousel Pagination Button.
...
well as from their own prop, and Slider is closed as a deliberate non-change (see its entry). **Select** (20) has since
been built, so what is in Figma and still unbuilt is **Autocomplete** and **Combobox** — both already
have a `Type` in the Field set, so Field is waiting for them rather than the other way round.
