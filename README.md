...
| **ResizeHandle** | The strip you drag to change a size — a focusable `separator` with `aria-valuenow`/`min`/`max`, vertical or horizontal, reporting a value in the caller's unit (spans on the dashboard, pixels on the rail). It is the gap it sits in, at the gap's width; the pill follows the pointer; arrows step, Shift steps further, Home and End go to the ends; `onResizeStart`/`onResizeEnd` bracket a pointer drag |
...
| **Skeleton** | Default/subtle × upright/inverse, any size or shape via `className`; `Skeleton.Text` at sm/base/lg/xl takes exactly the space of a line — pulses at shadcn's speed on the motion tokens |
| **ChatMessage** | One turn of a conversation: an avatar slot, stacked bubbles (default/emphasized/ghost × default/small, the tail on the sender's side) and a metadata row — delivery status and time for a sent message, an actions row and time for a received one, plus display-only reaction pills. `direction` is set once on the message and every part follows |
| **ThoughtProcess** | An assistant's reasoning folded under a quiet row — the app's mark and a rotating line in the system's own voice ("Measuring, not assuming", "Yet another pass") while it runs, "Thought summary" once it is done — opening onto a Card of tool calls. Built on Base UI's Collapsible |
| **ToolCall** | One thing the assistant did: a status glyph (pending/running/done/error) and a label; give it a `detail` and it becomes a disclosure |
| **ChatComposer** | The message field with its action row and send button; grows from one line to eight, Enter sends and Shift+Enter breaks the line, and `streaming` swaps send for stop. Default/small at 96/72 |
| **AppShell** | The frame an application sits in: canvas, 8px `frame`, a nav beside (`side`) or above (`top`) an `AppShell.Page` holding a `TopBar` and the scrolling `AppShell.Content`. `floating` sits the blocks on the canvas with the nav lifted; `contained` puts them in a bordered `surface-background-primary` panel with the nav flush — the nav takes its shadow default from the shell |
| **Outline** | A table of contents for the page: same-page links indented by heading level (`level` 1–6; `size` `default` 32px rows or `small` 24px, Tabs' two sizes), Tabs' 1px rule as a track and its 2px indicator that slides to the active heading. Scroll-spy built in, off when `activeId` is controlled; `offset` for a fixed header, `scrollContainerRef` for a panel; `useOutlineFromDOM` reads the items off a container's headings |
| **Panel** | A region beside the page that pushes it over: an in-flow `<aside>` written after (or before) `AppShell.Page`, 384 wide, with `Panel.Header` (ContentBlock's header plus a ×) and a scrolling `Panel.Body`. Non-modal — focus moves in on open and back out on close, Escape closes from inside. `resizable` puts a ResizeHandle on the seam (320–640); `floating` adds the low shadow, defaulted from the shell. Below 768 it stacks under the page at `defaultHeight` and the handle turns horizontal |
| **Drawer** | A surface that slides over the page from `side` right, left or bottom, on Base UI's Drawer: a scrim, a focus trap, swipe toward the edge to dismiss. `Drawer.Popup` (width 384) swallows Portal, Backdrop, Viewport and Content; `Drawer.Header` is ContentBlock's header with a `Drawer.Title` and a ×; `Drawer.Body` scrolls. `modal={false}` keeps the page live. Below 768 a side drawer becomes a bottom sheet |
| **Questionnaire** | An assistant asking before it acts, one question at a time: single-select rows, `multiple` boxes, a free-text alternative, Skip on an optional question, letter or number shortcuts on every row, Previous/Next/Submit shown as the open question changes. Built on `@shadcn/react/questionnaire`, the first form primitive here that is not Base UI's; answers come out of `FormData`, arrays for `multiple`. `Questionnaire.Recap` is the answered form — a Card of question / answer pairs that takes the questionnaire's place in the same turn, a skipped question still a line |
...
| **ResponsiveNav** | `TopNav` above 768px and `MobileNav` below it, swapped in CSS so it survives a server render and the first paint. Owns the `md:` breakpoint rather than making the caller write one. Takes `pages` and `sections` separately, because a top bar's handful of links and the sheet's full tree of headers and groups are not the same content |
| **SideNav** | The application rail; 224px expanded and 56px collapsed, with `SideNav.Section` (optional group header) and `SideNav.Group` (a disclosure built on Base UI's Collapsible). Collapsed, every label becomes both the accessible name and a tooltip, each section header becomes a 1px rule, and a group opens sideways as a 224px flyout on hover or click. `floating` toggles the drop shadow and nothing else. Logo and bottom-pinned utilities are slots. `resizable` puts a ResizeHandle on the seam — the shell's 8px gap — to set the expanded width between `minWidth` and `maxWidth` (192–400); resizing a collapsed rail opens it |
