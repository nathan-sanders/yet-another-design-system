...
| **SideNav** | The application rail; 224px expanded and 56px collapsed, with `SideNav.Section` (optional group header) and `SideNav.Group` (a disclosure built on Base UI's Collapsible). Collapsed, every label becomes both the accessible name and a tooltip, and each section header becomes a 1px rule. `floating` toggles the drop shadow and nothing else. Logo and bottom-pinned utilities are slots |
