...
| **Outline** | A table of contents for the page: same-page links indented by heading level (`level` 1–6; `size` `default` 32px rows or `small` 24px, Tabs' two sizes), a 2px track and an indicator that slides to the active heading. Scroll-spy built in, off when `activeId` is controlled; `offset` for a fixed header, `scrollContainerRef` for a panel; `useOutlineFromDOM` reads the items off a container's headings |
