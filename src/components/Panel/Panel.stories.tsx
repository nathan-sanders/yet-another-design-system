          ...
          <BentoGrid.Cell colSpan={2}>
            <Block title="Overview">
              <p className="text-content-subtle">
                Open the details panel from the bar. It pushes this page over
                rather than covering it, and you can work in either.
              </p>
            </Block>
          </BentoGrid.Cell>
          <BentoGrid.Cell>
...
function measure(canvasElement: HTMLElement) {
  ...
  const aside = () =>
    within(canvasElement).queryByRole("complementary", { name: "Details" });
  const card = () =>
    aside()?.firstElementChild?.firstElementChild as HTMLElement | undefined;
  const handle = () =>
...
export const PlaygroundDriven: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("closed, there is no panel and no gap", async () => {
      ...
      await expect(page.getBoundingClientRect().right).toBe(
        shell.getBoundingClientRect().right - 8,
      );
    });

    await step(
      "open, it is a landmark beside the page, 384 wide, 8 from it",
      async () => {
        ...
        await expect(panel.getBoundingClientRect().left).toBe(
          page.getBoundingClientRect().right + 8,
        );
    ...
    await step(
      ...
      async () => {
        ...
        await expect(getComputedStyle(panel.firstElementChild!).overflow).toBe(
          "visible",
        );
        await expect(
          getComputedStyle(panel.firstElementChild!.firstElementChild!)
            .boxShadow,
        ).not.toBe("none");
      },
    );
    ...
    await step(
      "Escape closes it from inside, and focus returns to the toggle",
      async () => {
...
export const ResizableKeyboard: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "the handle is the gap: 8 wide, from the page's edge to the panel's",
      async () => {
...
export const LeftKeyboard: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const panel = aside()!;
    const separator = handle()!;
    await expect(panel.getBoundingClientRect().right).toBe(
      page.getBoundingClientRect().left - 8,
    );
    const rect = separator.getBoundingClientRect();
        ...
        <AppShell.Content>
          <BentoGrid columns={2}>
            <BentoGrid.Cell colSpan={2}>
              <Block title="Overview">
                <p className="text-content-subtle">
                  Open the details from the More menu.
                </p>
              </Block>
            </BentoGrid.Cell>
          </BentoGrid>
...
export const InContextDriven: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "flush in a contained shell: a bordered card, no shadow",
      async () => {
        const style = getComputedStyle(card()!);
        await expect(style.borderTopLeftRadius).toBe("12px");
        await expect(style.borderTopWidth).toBe("1px");
...
export const TopNavigationKeyboard: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("the handle is horizontal at a desktop width", async () => {
      await expect(
        canvas.getAllByRole("separator", { name: "Resize Details" }),
      ).toHaveLength(1);
      const separator = handle()!;
      await expect(separator).toHaveAttribute("aria-orientation", "horizontal");
      await expect(separator.getBoundingClientRect().height).toBe(8);
      await expect(separator.getBoundingClientRect().top).toBe(
      ...
      await waitFor(() =>
        expect(panel.getBoundingClientRect().height).toBe(328),
      );
    });
  },
};
...
export const PhoneKeyboard: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "the handle is horizontal, on the seam above the panel",
      async () => {
        ...
        await expect(panel.querySelectorAll('[role="separator"]')).toHaveLength(
          2,
        );
          ...
          >
            ...
            <Panel.Body className="flex flex-col gap-4">
              ...
              >
                ...
                <Panel.Body className="flex flex-col gap-4">
                  <p className="text-content-subtle">
                    Reassign this report to Priya? They will be notified, and
                    you will lose the ability to edit it.
                  </p>
                  <div className="flex justify-end gap-2">
                    <Button
                      appearance="secondary"
...
function level(canvasElement: HTMLElement, name: string) {
  ...
  const card = aside?.firstElementChild?.firstElementChild as
...
export const StackedDriven: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "a control inside the panel opens a second one over it",
      async () => {
        await userEvent.click(
          canvas.getByRole("button", { name: "Edit owner" }),
        );
    ...
    await step(
      ...
      async () => {
        ...
        await expect(
          root().aside!.style.getPropertyValue("--nested-panels"),
        ).toBe("2");
        await expect(middle().aside).toHaveAttribute("data-nested-panel-open");
        await expect(front().aside).not.toHaveAttribute(
          "data-nested-panel-open",
        );
    ...
    await step(
      "a covered panel keeps its surface and loses its content",
      async () => {
        ...
        await waitFor(() =>
          expect(getComputedStyle(root().content!).opacity).toBe("0"),
        );
    ...
    await step(
      ...
      async () => {
        await userEvent.click(
          within(middle().aside!).getByRole("button", { name: "Close" }),
        );
        ...
        await expect(root().aside).not.toHaveAttribute(
          "data-nested-panel-open",
        );
        await expect(root().content).toHaveProperty("inert", false);
        await waitFor(() =>
          expect(getComputedStyle(root().content!).opacity).toBe("1"),
        );
