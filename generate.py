...
out.append("/*                                                                           */")
out.append("/*     A keyframe is for motion with no state change to transition between  */")
out.append("/*     — a phrase that swaps under the same element, a loop. Anything that   */")
out.append("/*     opens or closes is a CSS transition on Base UI's starting and ending  */")
out.append("/*     styles instead, and lives in its component's recipe.                 */")
out.append("/*                                                                           */")
out.append("/*     `fade-in` is the first: ThoughtProcess's phrase crossfade, `fast`      */")
out.append("/*     and `both`. It was also MobileNav's entrance until 2026-09-19, paired  */")
out.append("/*     with a `slide-out-to-bottom` that left with it — the sheet is now a    */")
out.append("/*     Drawer, and slides both ways on Drawer's transition. See               */")
out.append("/*     src/components/Nav/CLAUDE.md for why it was ever a fade.              */")
out.append("@keyframes fade-in {")
out.append("  from { opacity: 0; }")
out.append("  to   { opacity: 1; }")
out.append("}")
out.append("")
out.append("/*     `progress-indeterminate` is the second, and the first that loops. A    */")
out.append("/*     40%-wide fill crosses the track from just off one end to just off     */")
out.append("/*     the other; Astryx draws the same sweep. It is a keyframe rather      */")
out.append("/*     than a transition for the ordinary reason — there is no state        */")
out.append("/*     change to transition between.                                        */")
out.append("/*                                                                          */")
out.append("/*     `linear`, not `--ease-standard`. That curve is a fast-out easing for  */")
out.append("/*     something arriving or leaving, and a loop with no beginning and no    */")
out.append("/*     end has neither; eased, the sweep visibly hesitates at both edges     */")
out.append("/*     of every cycle. The duration is a token — `slow-max`, 1300ms, the     */")
out.append("/*     nearest step to Astryx's 1500 — because that is the rule, and the     */")
out.append("/*     scale has no 1500.                                                    */")
out.append("/*                                                                          */")
out.append("/*     Section 5 below clamps this to 1ms with one iteration under           */")
out.append("/*     prefers-reduced-motion, which would strand the fill off the end of    */")
out.append("/*     the track. ProgressBar turns the animation off instead — see          */")
out.append("/*     src/components/ProgressBar/CLAUDE.md.                                 */")
out.append("@keyframes progress-indeterminate {")
out.append("  from { transform: translateX(-100%); }")
out.append("  to   { transform: translateX(250%); }")
out.append("}")
out.append("")
out.append("/*     `skeleton-pulse` is the third, and the second that loops. A placeholder */")
out.append("/*     breathes between full and half opacity; shadcn draws the same pulse   */")
out.append("/*     over a literal 2s, and this one runs `slow` (975ms) each way with      */")
out.append("/*     `alternate`, so a cycle is 1950ms — the same speed, spelled as a token.*/")
out.append("/*     `ease-in-out` is a CSS keyword like the `linear` above: a breath slows */")
out.append("/*     at both ends, and `--ease-standard` only slows at one. Not Tailwind's  */")
out.append("/*     own `animate-pulse`, whose 2s and bezier are literals Figma cannot     */")
out.append("/*     reach. Skeleton adds `motion-reduce:animate-none` for the same reason  */")
out.append("/*     ProgressBar does — see src/components/Skeleton/CLAUDE.md.              */")
out.append("@keyframes skeleton-pulse {")
out.append("  from { opacity: 1; }")
out.append("  to   { opacity: 0.5; }")
out.append("}")
out.append("")
out.append("/*     `text-shimmer` is the fourth, and the third that loops. A band of a    */")
out.append("/*     second color sweeps once across a run of text, the way an assistant  */")
out.append("/*     UI marks a line it is still working on. The element paints a         */")
out.append("/*     gradient twice its own width, clipped to its glyphs, and this walks  */")
out.append("/*     that gradient from one end to the other; the colors are the          */")
out.append("/*     element's own (`from-`/`via-` utilities on the content tokens), so   */")
out.append("/*     the keyframe carries none. `linear` and `slow-max`, for the reasons  */")
out.append("/*     progress-indeterminate gives — a loop with no ends wants no easing,  */")
out.append("/*     and 1300ms is the longest step. ThoughtProcess is the only user, and */")
out.append("/*     it turns the sweep off under reduced motion and paints the text      */")
out.append("/*     flat, because the 1ms clamp below would strobe it.                   */")
out.append("@keyframes text-shimmer {")
out.append("  from { background-position: 100% 0; }")
out.append("  to   { background-position: -100% 0; }")
out.append("}")
out.append("")
out.append("@theme {")
out.append("  --animate-fade-in: fade-in "
           "var(--transition-duration-fast) var(--ease-standard) both;")
out.append("  --animate-progress-indeterminate: progress-indeterminate "
           "var(--transition-duration-slow-max) linear infinite;")
out.append("  --animate-skeleton-pulse: skeleton-pulse "
           "var(--transition-duration-slow) ease-in-out infinite alternate;")
out.append("  --animate-text-shimmer: text-shimmer "
           "var(--transition-duration-slow-max) linear infinite;")
out.append("}")
out.append("")
...
out.append("}")
out.append("")
out.append("/* ---- 7. Color scheme. Tells the browser which theme the page is in, so   */")
out.append("/*    what it draws itself follows the tokens: scrollbars first of all,     */")
out.append("/*    and native pieces like date pickers and autofill. Without it, a       */")
out.append("/*    dark page got light-mode scrollbars — a dark-gray overlay thumb on   */")
out.append("/*    the dark Combobox panel, and a light track wherever scrollbars are   */")
out.append("/*    always shown. `.dark` rather than `html.dark` so it matches the      */")
out.append("/*    semantic block in section 2, the same class the theme toggle sets.   */")
out.append(":root {")
